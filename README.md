...
For the pCR predictions, the ANN model yielded the best results with a 5-fold cross validation score of 80% (good accuracies in predicting cancer not returning but low accuracies in predicting cancer returning as dataset was imbalanced causing outputs to also be imbalanced). For the RFS predictions, the ANN model yielded the best results with a mean absolute error of 21.28 in a 5-fold cross validation.
