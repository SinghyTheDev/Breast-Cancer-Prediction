...
## Feature Selection
Feature selection starts with visualisations to identify relevant features and correlations. Principal Component Analysis (PCA) is used to reduce dimensionality. Three wrapper methods — variance threshold, univariate feature selection, and mutual information feature selection are used to reduce dimensionality for the RFS task. Selecting k-best attributes with ANOVA F-value scoring was used for feature selection in the pCR classification task.

## PCR (Classification Task)
Four ML models are compared for predicting pCR: Artificial Neural Network, Logistic Regression, Support Vector Machine, and Decision Tree. These models are evaluated using classification accuracy, confusion matrices, and F1 score. The ANN model is recommended as it yields the best results.
