...
For the pCR predictions, the ANN model yielded the best results with a 5-fold cross validation score of 80% (high accuracies in predicting non-returning cancers but lower accuracies in predicting returning cancer as dataset was imbalanced causing results to also be imbalanced). For the RFS predictions, the ANN model yielded the best results with a mean absolute error of 21.28 in a 5-fold cross validation.
